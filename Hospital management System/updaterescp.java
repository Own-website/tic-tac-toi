...
public class updaterescp implements ActionListener {
...
public updaterescp() 
	...
	public static void main(String[] args) {
		updaterescp window = new updaterescp();
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(YesorNo==0)
		{
			...
			String gen="male";
				...
				Connection cn=Dao.mycon();
				PreparedStatement ps=cn.prepareStatement("update rescption set id=(?),Fname=(?),Sname=(?),Age=(?),Qualification=(?),Gender=(?),address=(?),city=(?),phno=(?),Mstatus=(?),jdate=(?),ldate=(?),uname=(?),Upass=(?)");
				ps.setString(1,id);
