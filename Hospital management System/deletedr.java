...
public class deletedr implements ActionListener {
	...
	JButton bsearch,back,blogout,bdelete;
	JRadioButton r1,r2;
	JComboBox cb1;
...
public deletedr() 
	{
		...
		bdelete.setFont(new Font("Tahoma", Font.BOLD, 20));
		bdelete.setForeground(SystemColor.textHighlight);
		bdelete.setBackground(SystemColor.menu);
		bdelete.setBounds(1201, 114, 111, 40);
		...
		if(e.getSource()==bdelete)
		{
			...
			if(YesorNo==0)
			{
				try {
					...
					//int b1=Integer.parseInt(a1);
					Connection cn=Dao.mycon();
					PreparedStatement ps=cn.prepareStatement("delete from doctor where id=(?)");
					ps.setString(1,a1);
					ps.executeUpdate();
					frame.dispose();
					new frontpag();
					...
					System.out.println(e1);
					JOptionPane.showMessageDialog(bdelete, "Somthing went wrong");
				}	
			}
		}
	}
