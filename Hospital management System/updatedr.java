...
public class updatedr implements ActionListener {
	...
	JButton bsearch,back,blogout,bupdate;
	JRadioButton r1,r2;
	JComboBox cb1;
...
public updatedr() 
	{
		...
		l1 = new JLabel("Delete Resceptionist");
		...
		bupdate.setForeground(SystemColor.textHighlight);
		bupdate.setBackground(SystemColor.menu);
		bupdate.setBounds(1201, 114, 139, 40);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		try{
			...
			PreparedStatement st=cn.prepareStatement("select * from doctor where id="+b1+"");
		...
		if(YesorNo==0)
		{
			try {
				...
				String qul=tqualification.getText();
			...
			int agg=Integer.parseInt(age);
			String uname=tuname.getText();
			String upass=tupassword.getText();
			...
			String gen="male";
				...
				Connection cn=Dao.mycon();
				PreparedStatement ps=cn.prepareStatement("update doctor set id=(?),Fname=(?),Sname=(?),Age=(?),Qualification=(?),Gender=(?),address=(?),city=(?),phno=(?),Mstatus=(?),jdate=(?),ldate=(?),uname=(?),Upass=(?)");
				ps.setString(1,id);
