...
public class deletpatient implements ActionListener{
	char a[];
	...
	private JButton blogout,back,search,bdelet;
	...
	public deletpatient() {
		...
		bdelet.setForeground(SystemColor.text);
		bdelet.setBackground(SystemColor.textHighlight);
		bdelet.setFont(new Font("Tahoma", Font.BOLD, 20));
		bdelet.setBounds(1225, 140, 115, 40);
		frame.getContentPane().add(bdelet);
		bdelet.addActionListener(this);
	}
	public static void main(String[] args) {
		deletpatient window = new deletpatient();
	...
	@Override
		...
		//dilog message for logout button
		...
		//search the data and get the data into text field
		if(e.getSource()==search)
		{
			...
			try {
				...
				PreparedStatement st=cn.prepareStatement("select * from addpatient where pid="+b1+"");
				...
				//for dob to breake the string into index
			...
			catch (SQLException e1)
			{
				...
				e1.printStackTrace();
				JOptionPane.showMessageDialog(search, "invalid");
			}
			
			}
			if(e.getSource()==bdelet)
			{
				int YesorNo=JOptionPane.showConfirmDialog(null, "Do you really want to delete data","delete", JOptionPane.YES_NO_OPTION);
				if(YesorNo==0)
				{
					try {
						String a1=tid.getText();
						//int b1=Integer.parseInt(a1);
						Connection cn=Dao.mycon();
						PreparedStatement ps=cn.prepareStatement("delete from addpatient where pid=(?)");
						ps.setString(1,a1);
						ps.executeUpdate();
						}
					catch(Exception e1){
						System.out.println(e1);
						JOptionPane.showMessageDialog(bdelet, "Somthing went wrong");
					}
					frame.dispose();
					new frontpag();
				}
